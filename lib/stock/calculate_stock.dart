...
class CalculateStock {
  static List<AllItem> mergeFabricAndItem(
      List<Item> items, List<Fabric> fabrics) {
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      allItems.add(AllItem(
          ...
          category: 'item',
          item: item,
          dateTime: DateTime(int.parse(item.year), int.parse(item.month),
              int.parse(item.day))));
    }
    for (int i = 0; i < fabrics.length; i++) {
      ...
      allItems.add(AllItem(
          ...
          category: 'fabric',
          fabric: fabric,
          dateTime: DateTime(int.parse(fabric.year), int.parse(fabric.month),
              int.parse(fabric.day))));
    }
    return allItems;
  }
  ...
  static void sortFabric(List<Fabric> fabrics) {
    fabrics.sort((a, b) => DateTime(
            a.year.parseInt(), a.month.parseInt(), a.day.parseInt())
        .compareTo(
            DateTime(b.year.parseInt(), b.month.parseInt(), b.day.parseInt())));
  }

...
class StreamPageProvider extends ChangeNotifier{
  ...
  bool isEqual(TimerStreamer timerStreamer){
    bool check = true;
    if(timerStreamer.monitorItemController.length > this.timerStreamer.monitorItemController.length){
      print("3-1");
      check = false;
    }else{
      for(int i = 0 ; i < this.timerStreamer.monitorItemController.length;i++){
        AssignPersonnel a = this.timerStreamer.monitorItemController[i].startAssign.assignPersonnel;
        AssignPersonnel b = timerStreamer.monitorItemController[i].startAssign.assignPersonnel;

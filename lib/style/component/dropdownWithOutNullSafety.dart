...
class _CustomDropdownMenuState<T> extends State<_CustomDropdownMenu<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    // ];
    for(int itemIndex = 0; itemIndex < route.items.length; ++itemIndex){
      children.add(  _CustomDropdownMenuItemButton<T>(
        route: widget.route,
        padding: widget.padding,
        buttonRect: widget.buttonRect,
        constraints: widget.constraints,
...
class _CustomDropdownRouteResult<T> {
  ...
  bool operator ==(Object other) {
    return other is _CustomDropdownRouteResult<T>
        && other.result == result;
  }
...
class _CustomDropdownRoute<T> extends PopupRoute<_CustomDropdownRouteResult<T>> {
...
class _CustomDropdownButtonState<T> extends State<CustomDropdownButton<T>> with WidgetsBindingObserver {
  ...
  void _removeDropdownRoute() {
    _dropdownRoute != null??_dropdownRoute._dismiss();
    _dropdownRoute = null;
  ...
  void _handleTap() {
    ...
    // List<_MenuItem<T>> menuItems = <_MenuItem<T>>[];
    // for (int index = 0; index < widget.items.length; index += 1){
    //   menuItems.add(MenuItem<T>(
    ...
    navigator.push(_dropdownRoute).then<void>((_CustomDropdownRouteResult<T> newValue) {
      _removeDropdownRoute();

...
class NewProjectSizeCubit extends Cubit<NewProjectSizeState> {
        ...
        children: [
          TextButton(
            onPressed: () {
              ...
              emit(NewProjectSizeState(
                  sizeTexts: state.sizeTexts, sizeWidgets: state.sizeWidgets));
            },
            child: Container(
              ...
              child: Center(
                  child: Text(
                MyIcons.CANCEL,
                style: MyTextStyle.iconStyle.copyWith(fontSize: 15),
              )),
            ),
          ),
          DefaultTextField(
  ...
  void add() {
    if (state.sizeWidgets == null && state.sizeTexts == null) {
      ...
      state.sizeTexts = [];
      state.sizeWidgets = [];
      MyTextEditKeyValue textEditKeyValue = new MyTextEditKeyValue(
          key: newLength, textEditingController: TextEditingController());
      state.sizeTexts.add(textEditKeyValue);
      MyWidgetKeyValue widgetKeyValue =
          new MyWidgetKeyValue(key: newLength, widget: sizeWidget(newLength));
      state.sizeWidgets.add(widgetKeyValue);
      emit(NewProjectSizeState(
          sizeWidgets: state.sizeWidgets, sizeTexts: state.sizeTexts));
    } else {
